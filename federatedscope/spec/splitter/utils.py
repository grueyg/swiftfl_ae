...
def split_according_to_prior(label, client_num, prior):
...
def dirichlet_distribution_noniid_slice(label,
    ...
    p = [0.3, 0.5, 0.7, 0.8]
    n = [int(0.5*client_num), int(0.25*client_num), 
         client_num - int(0.5*client_num) - int(0.25*client_num)]
    alpha_p1 = np.random.uniform(alpha, p[0], n[0])
    alpha_p2 = np.random.uniform(p[1], p[2], n[1])
    alpha_p3 = np.random.uniform(p[3], 1, n[2])
    alpha_array = np.concatenate([alpha_p1, alpha_p2, alpha_p3])
    alpha_array = alpha_array / alpha_array.sum()
    alpha_array = np.random.uniform(alpha, 1, client_num)
    ...
    while size < min_size:
        ...
        for k in range(classes):
            ...
            prop = np.random.dirichlet(alpha_array)
            # prop = np.array([
    ...
    label_slice = [[] for _ in range(len(idx_slice))]
    for i in range(int(client_num)):
        label_slice[i] = label[idx_slice[i]]

    return idx_slice, label_slice

def split_list_by_normal_distribution(lst, n, mean, std):
    N = len(lst)

    # 计算每个子列表的期望值和方差
    expected_counts = np.random.normal(mean, std, n)
    expected_counts *= N / np.sum(expected_counts)  # 根据总数量 N 进行缩放

    # 将期望数量分配到每个子列表中
    counts = np.random.multinomial(N, expected_counts / np.sum(expected_counts))
