...
class LEAF_TWITTER_MERGED(LEAF):
    """
    ...
    def __init__(self,
        ...
        if name != 'twitter_merged':
            raise ValueError('`name` should be `twitter_merged`.')
        else:
        ...
        super(LEAF_TWITTER_MERGED, self).__init__(root, name, transform,
                                           target_transform)
    ...
    def process(self):
        ...
        client_sample_num_list = self.generate_power_law_distribution(10000, self.client_num, a=2)
        idx = 0
        for num, file in enumerate(files):
            ...
            user_list = user_list[:16900]

            tmp_data  = []
            tmp_targets = []
            for user in tqdm(user_list):
                if idx >= self.client_num:
                    break
                ...
                if len(tmp_data) >= client_sample_num_list[idx]:
                    data = torch.LongTensor(np.stack(tmp_data))
                    targets = torch.LongTensor(np.stack(tmp_targets))
    ...
    def generate_power_law_distribution(self, total_samples, num_clients, a):
        """
        Generate a power law distribution of samples among a number of clients.
        
        Parameters:
        - total_samples: int, the total number of samples to distribute
        - num_clients: int, the number of clients among which to distribute samples
        - a: float, the exponent of the power law distribution
        
        Returns:
        - samples_per_client: list of int, the number of samples for each client
        """
        # 生成符合幂律分布的随机样本
        samples_per_client = np.random.power(a, size=num_clients)
        
        # 归一化样本数
        samples_per_client *= total_samples / np.sum(samples_per_client)
        
        # 确保样本总数不变，并且为整数
        samples_per_client = np.round(samples_per_client).astype(int)

        # 修正由于四舍五入导致的样本数总和不等于16900的问题
        while np.sum(samples_per_client) != total_samples:
            difference = total_samples - np.sum(samples_per_client)
            # 如果样本过多，从最大的客户端中减去
            if difference < 0:
                samples_per_client[np.argmax(samples_per_client)] += difference
            # 如果样本过少，添加到最小的客户端
            else:
                samples_per_client[np.argmin(samples_per_client)] += difference
        
        return samples_per_client.tolist()
