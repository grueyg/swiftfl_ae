...
def setup_multigpu_runner(cfg, server_class, client_class, unseen_clients_id,
                          server_resource_info, client_resource_info, client_cfgs):
    processes = []
...
class SpecStandaloneMultiGPURunner(SpecStandaloneRunner):
    def _set_up(self):
    ...
    def _get_server_args(self, resource_info=None, client_resource_info=None):
        if self.server_id in self.data:
            ...
            client_info = copy.deepcopy(client_resource_info)
            for client, size in zip(client_info, data_size):
                client['computation'] = client['computation'] * size * self.cfg.federate.augmentation_factor
                client['communication'] = client['communication'] / 2
        kw = {
            'shared_comm_queue': self.server2client_comm_queue,
            'id2comm': self.id2comm,
            'resource_info': resource_info,
            'client_resource_info': client_info
        }
    ...
    def run(self):
        ...
        setup_multigpu_runner(self.cfg, self.server_class, self.client_class,
                              ...
                              client_resource_info, self.client_cfgs)
...
class ServerRunner(Runner):
    ...
    def setup(self):
        ...
        self.server = self.server_class(
            ...
            total_round_num=self.config.federate.total_round_num,
            device=self.device,
...
class ClientRunner(Runner):
    def __init__(self, rank, client_ids, config, client_class,
                 ...
                 client_resource_info, client_cfgs):
        super().__init__(rank)
    ...
    def setup(self):
        ...
        for client_id in self.client_ids:
            ...
            kw = {
                'shared_comm_queue': self.client2server_comm_queue,
                'resource_info': self.client_resource_info[client_id - 1]
