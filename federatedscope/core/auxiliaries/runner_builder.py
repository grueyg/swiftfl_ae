...
from federatedscope.spec.parallel.parallel_runner import \
    SpecStandaloneMultiGPURunner
...
def get_runner(server_class,
    ...
    if mode == 'standalone':
        if process_num <= 1:
            ...
            runner_cls = SpecStandaloneRunner # add client_sample_size * computation in client_info
        else:
            # runner_cls = StandaloneMultiGPURunner
            runner_cls = SpecStandaloneMultiGPURunner
    ...
    if runner_cls is SpecStandaloneMultiGPURunner:
        data = None
