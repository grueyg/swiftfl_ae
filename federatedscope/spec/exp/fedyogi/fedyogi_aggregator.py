...
class FedYogiAggregator(ClientsAvgAggregator):
    """
    ...
    def __init__(self, config, model, device='cpu'):
        super(FedYogiAggregator, self).__init__(model, device, config)
        self.optimizer = get_optimizer(model=self.model,
                                       **config.fedyogi.optimizer)
        if config.fedyogi.annealing:
            self._annealing = True
            # TODO: generic scheduler construction
            self.scheduler = torch.optim.lr_scheduler.StepLR(
                self.optimizer,
                step_size=config.fedyogi.annealing_step_size,
                gamma=config.fedyogi.annealing_gamma)
        else:
