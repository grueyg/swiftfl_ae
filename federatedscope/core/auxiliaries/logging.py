...
def update_logger(cfg, clear_before_add=False):
    ...
    if cfg.expname == "":
        ...
        #               f"_{cfg.data.type}_lr{cfg.train.optimizer.lr}_lste" \
        #               f"p{cfg.train.local_update_steps}"
        cfg.expname = f"{cfg.federate.method}" \
                      f"_{cfg.model.type}" \
                      f"_lr{cfg.train.optimizer.lr}" \
                      f"_lstep{cfg.train.local_update_steps}" \
                      f"_C{cfg.federate.client_num}" \
                      f"_S{cfg.federate.sample_client_num}" \
                      f"_R{cfg.federate.total_round_num}"
    if cfg.expname_tag:
        cfg.expname = f"{cfg.expname}_{cfg.expname_tag}"
    cfg.outdir = os.path.join(cfg.outdir, cfg.data.type, cfg.expname)
